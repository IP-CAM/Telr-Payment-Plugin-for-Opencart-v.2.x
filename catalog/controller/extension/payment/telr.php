...
class ControllerExtensionPaymentTelr extends Controller {
	...
	public function index() {
		...
		$testmode=$this->_testmode($this->config->get('telr_test'));
		$amount= $this->currency->format(
			$order_info['total'],
			$order_info['currency_code'],
			$order_info['currency_value'],
			false);
		$cart_desc=trim($this->config->get('telr_purdesc'));
		if (empty($cart_desc)) {
		...
		$post_data = Array(
			'ivp_method'		=> 'create',
			'ivp_authkey'		=> $this->config->get('telr_authkey'),
			'ivp_store'		=> $this->config->get('telr_store'),
			'ivp_lang'		=> $this->config->get('telr_lang'),
			'ivp_cart'		=> $cart_id,
			'ivp_amount'		=> $amount,
		...
		if ($this->_isHttps() && $this->config->get('telr_pay_mode') == '2') {
		...
		if(isset($returnData['order'])) {
			...
			$_SESSION['telr_orderref']=$jobj['ref'];
			$redirurl=$jobj['url'];
	...
	public function payment_authorised($order_id,$txref) {
		...
		$order_status = $cart_desc=trim($this->config->get('telr_comp_status_id'));
	...
	public function payment_cancelled($order_id,$txref) {
		...
		$order_status = $cart_desc=trim($this->config->get('telr_void_status_id'));
		if (empty($order_status)) {
	...
	public function payment_pending($order_id,$txref) {
		...
		$order_status = $cart_desc=trim($this->config->get('telr_pend_status_id'));
		if (empty($order_status)) {
	...
	public function callback_return() {
		if ($this->check_payment($_SESSION['telr_orderref'])) {
			// Authorised
	...
	public function callback() {
		if(isset($_SESSION['telr_orderref'])) {
